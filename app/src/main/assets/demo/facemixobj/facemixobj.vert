...
attribute vec3 aKaikouPosition;  //顶点位置
attribute vec3 aPiezuiPosition;  //顶点位置
attribute vec3 aYoupiezuiPosition;  //顶点位置
uniform vec4 uPositionWeight;//各个顶点位置的权重
...
void main()
{
    //vec3 finalPosition=aPosition;//aYoupiezuiPosition;//aPiezuiPosition;//aKaikouPosition;//aPosition;
    //vec3 finalPosition=mix(aKaikouPosition,aYoupiezuiPosition,0.5);
    //vec3 finalPosition=aPosition*uPositionWeight.x+aKaikouPosition*uPositionWeight.y+aPiezuiPosition*uPositionWeight.z+aYoupiezuiPosition*uPositionWeight.w;
    vec3 test1=mix(aPosition,aKaikouPosition,uPositionWeight.x);
    vec3 test2=mix(aPiezuiPosition,aYoupiezuiPosition,uPositionWeight.y);
    vec3 finalPosition=mix(test1,test2,uPositionWeight.z);
